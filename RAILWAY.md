...
| `R2_PUBLIC_BASE`                  | no       | Public Cloudflare R2 base URL for v6 image relpaths (e.g. `https://pub-070ce3c008c44a0e95c52fb69250b046.r2.dev/ocr_omni`). When set, the agent prepends it to `exercise_*_image_relpath` and `exam_full_*_relpath` so the frontend can render figures directly via `<img>`. When absent, raw relpaths are returned (the frontend must then compose its own URL or skip rendering). The bucket is fully public for pre-launch dev — harden with hotlink protection or a Cloudflare Worker before opening to students. |
